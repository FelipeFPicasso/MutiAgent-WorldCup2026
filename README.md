...
> ⚠️ **Importante:** o projeto foi testado e validado nas versões **Python 3.10.12** e **Python 3.11**. O funcionamento correto das dependências (em especial `torch`, `chromadb` e `sentence-transformers`) é garantido apenas nessas versões.
>
> Foi identificada **incompatibilidade ao utilizar versões mais recentes do Python** podendo ocorrer falhas na instalação ou execução das dependências. Recomenda-se fortemente utilizar uma das versões testadas (3.10.12 ou 3.11) ao criar o ambiente virtual.
...
> Certifique-se de utilizar Python 3.10.12 ou 3.11 (ex.: `python3.11 -m venv .venv`) para evitar problemas de incompatibilidade.
...
> Certifique-se de utilizar Python 3.10.12 ou 3.11 (ex.: `py -3.11 -m venv .venv`) para evitar problemas de incompatibilidade.
